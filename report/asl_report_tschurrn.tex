...
%This can be explained that the middleware is the bottleneck with only 4 threads and with 8 threads and more the database is the bottleneck. Because the middleware server instance has 8 cores, it performs much better with 8 or more threads than with 4. In order to prove this statements I will  show that the clients never are the bottleneck. This can be done by showing that the response time increases, because this means the clients do nothing but waiting during this time. Then I show that the middleware is the bottleneck when it runs with only 4 connections. This can be shown when the database response time is. I therefore will show that with 32 threads the queueing time is only $1/4$ of the queuing time with 8 threads. This is the case because the more requests wait when holding a database connection instead of in the queue in the 32 thread setup.
